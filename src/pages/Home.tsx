...
export function Home() {
  ...
  const startRandom = () => navigate('/quiz?mode=random')
  const startWrong = () => navigate('/quiz?mode=wrong')
  const startCategory = (category: QuestionCategory) =>
    navigate(`/quiz?mode=category&category=${category}`)

  return (
    <div className="mx-auto max-w-2xl px-4 py-6 sm:py-10">
      <header className="mb-8 text-center">
        <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">중대산업재해 문제풀이</h1>
        <p className="mt-2 text-sm text-slate-600 sm:text-base">
          중대재해처벌법 · 시행령 · 별표1 기반 4지선다 객관식
        </p>
        <p className="mt-1 text-xs text-slate-400">총 {totalQuestions}문항 · 학습 보조 도구</p>
      </header>
      ...
      <section>
        <h2 className="mb-3 text-sm font-semibold text-slate-700">카테고리별 학습</h2>
