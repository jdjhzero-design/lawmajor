...
import type { QuestionCategory, QuizResult } from '../types/question'
...
export function Result() {
  ...
  const { result, mode, category } = state
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 sm:py-10">
      <div className="mb-8 rounded-2xl border border-slate-200 bg-white p-6 text-center shadow-sm">
        ...
        {category && (
          <p className="mt-1 text-sm text-slate-500">{getCategoryTitle(category)}</p>
        )}
          ...
          <div className="flex flex-col gap-3">
            {result.wrongIds.map((id) => {
              ...
              return (
                ...
                >
                  <p className="mb-1 text-xs font-medium text-red-600">{q.id}</p>
                  <p className="mb-2 text-sm font-medium text-slate-900">{q.question}</p>
                  <p className="text-sm text-green-700">
                    정답: {['A', 'B', 'C', 'D'][q.answerIndex]}. {q.choices[q.answerIndex]}
                  </p>
