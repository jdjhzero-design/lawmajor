...
import { getAllQuestions, getQuestionsByCategory, getQuestionsByIds, shuffleQuestions } from '../lib/questions'
...
export function useQuiz({ mode, category, wrongIds = [] }: UseQuizOptions) {
  const questions = useMemo(() => {
    ...
    if (mode === 'category' && category) {
      return shuffleQuestions(getQuestionsByCategory(category as Question['category']))
    }
    return shuffleQuestions(getAllQuestions())
  }, [mode, category, wrongIds])
